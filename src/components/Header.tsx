...
const Header = () => {
  ...
  const navigation = [
    ...
    { name: "Surf", href: "#surf" },
    { name: "Hospedaje", href: "#accommodation" },
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-b border-surf-blue-100/50 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          ...
          <Link
            href="#packages"
            className="hidden lg:block bg-[#FFDA78] hover:bg-[#FFDA78]/90 text-[#0E3244] font-bold px-6 py-2 rounded-lg transition-all duration-300 shadow-md hover:shadow-lg transform hover:scale-105"
          >
            Reservar
          </Link>
            ...
            {isSignedIn ? (
              <div className="flex items-center space-x-1">
                  ...
                  appearance={{
                    elements: {
                      ...
                      userButtonPopoverActionButton:
                        "hover:bg-surf-light text-surf-navy",
                      userButtonPopoverActionButtonText:
                        "text-surf-navy font-medium",
                    },
                  }}
                />
                {user?.publicMetadata?.role === "admin" && (
                  <Link
                    href="/admin"
                    className="flex items-center space-x-1 text-surf-navy-600 hover:text-surf-blue-600 transition-colors px-2 py-1 rounded-md hover:bg-surf-blue-50"
                  >
                    <Settings size={14} />
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && (
        <div className="lg:hidden bg-white/95 backdrop-blur-xl border-t border-surf-blue-100/50 shadow-lg animate-slide-down">
              ...
              {isSignedIn ? (
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <Link
                      href="/profile"
                      className="flex items-center space-x-2 text-surf-navy-700 hover:text-surf-blue-600 font-medium py-2 px-3 rounded-lg hover:bg-surf-blue-50 transition-all duration-300"
                    >
                      <User size={16} />
